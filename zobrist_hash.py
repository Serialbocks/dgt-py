...
class ZobristHash:
    @staticmethod
    def hashBoard(board):
        return ZobristHash.hashPieces(board) ^ ZobristHash.hashCastling(board) ^ ZobristHash.hashTurn(board) ^ ZobristHash.hashEnPassant(board)

    @staticmethod
    def hashPieces(board):
        hash = 0
        occupied = board.occupied
        while occupied != 0:
            sq = Bitboard.lsb(occupied)
            hash ^= ZobristHash.hashPiece(sq, board.whiteAt(sq), board.roleAt(sq))
            occupied &= occupied - 1
        return hash

    @staticmethod
    def hashPiece(square, color, role):
        if color:
            index = role.index * 2 + 1
        else:
            index = role.index * 2
        return POLYGLOT[64 * index + square]

    @staticmethod
    def hashCastling(board):
        hash = 0
        cr = board.castlingRights
        if Bitboard.contains(cr, Square.H1):
            hash ^= POLYGLOT[768]
        if Bitboard.contains(cr, Square.A1):
            hash ^= POLYGLOT[768 + 1]
        if Bitboard.contains(cr, Square.H8):
            hash ^= POLYGLOT[768 + 2]
        if Bitboard.contains(cr, Square.A8):
            hash ^= POLYGLOT[768 + 3]
        return hash

    @staticmethod
    def hashTurn(board):
        return POLYGLOT[780] if board.turn else 0

    @staticmethod
    def hashEnPassant(board):
        return POLYGLOT[772 + Square.file(board.epSquare)] if board.hasLegalEnPassant() else 0
